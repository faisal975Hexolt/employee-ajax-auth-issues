    ...
    <table class="table table-bordered text-nowrap" style="width:100%;">
        ...
        @inject('settle', 'App\SettlementBrief')
            @if(count($transactions)>0)
                ...
                <tr>
                    <td>{{++$table_count}}</td>
                       <td>{{\Carbon\Carbon::parse($transaction->created_date)->format('M d, Y h:i:s A')}}</td>
                      {{-- <td>{{$transaction->merchant_gid}}</td>
                    <td>{{$transaction->name}}</td> --}}
                    <td >{{$transaction->id}}</td>
                    <td style="text-align:left;" id="transactiongid"><a href="javascript:transactionDetailsView('{{$transaction->transaction_gid}}')">{{$transaction->transaction_gid}}</a></td>
                    <td style="text-align:left;">{{$transaction->merchantorderId}}</td>
                    
                    <td>{{$transaction->transaction_amount}}</td>
                    <td  style="text-align:left;">{{\Carbon\Carbon::parse($transaction->transaction_date)->format('M d, Y h:i:s A')}}</td>
                    ...
                    @if($transaction->settlement_brief_gid)
                         <?php $settle_details= $settle->get_details($transaction->settlement_brief_gid);?> 
...
<div class="table-responsive">
    <table class="table table-bordered">
        ...
        <tbody id="ordertable">
            ...
            <tr>
                <td>{{++$table_count}}</td>
                <td  style="text-align:left; padding:left:10px;"><a href="javascript:orderDetails('{{$order->id}}')">{{$order->order_gid}}</a></td>
        ...
        <tbody id="itemtable">
            @if(count($items)>0)
                ...
                <tr>
                    ...
                    <td>
                        <a class="btn btn-danger btn-sm" href="javascript:deleteItem('{{$item->id}}')">Delete</a>
                    </td>
                </tr>
                @endforeach
            @else
    ...
    <tbody id="invoicetable">
        ...
        <tr>
            ...
            @if($invoice->invoice_status == 'saved')
                <td>
                    <a href="{{url('/')}}/merchant/invoice/edit/{{$invoice->id}}">{{$invoice->invoice_gid}}</a>
                </td>
            @else
    ...
    <tbody id="customertable">
        @if(count($customers)>0)
            ...
            <tr>
                ...
                <td>
                    <a class="btn btn-danger btn-sm" href="javascript:deleteCustomer('{{$customer->id}}')">Delete</a>
                </td>
            </tr>
            @endforeach
        @else
...
<table class="table table-bordered">
    ...
    <tbody id="customertable">
        @if(count($notifications)>0)
            ...
            <tr>
                <td>{{++$table_count}}</td>
                <td><span class="notifymodalclass" data-sr="{{$table_count}}" data-title="{{$notification->notify_type}}" data-content="{{$notification->message}}">{{$notification->message}}</span></td>
