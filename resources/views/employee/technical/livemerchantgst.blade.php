            ...
            <div class="panel-body">
                <div class="tab-content">
                    ...
                    <div id="{{str_replace(' ','-',strtolower($value->link_name))}}" class="tab-pane fade in active">
                        <div class="row">
                            <form id="gst-merchant-download-form" action="{{route('briefdwnld')}}" method="POST" role="form">
                                @csrf
                        ...
                        <div class="row">
                            <form id="gst-status-merchant-form" >
                                @csrf
                                ...
                                <div class="row" style="margin-top:15px;">
                                    <div class="col-sm-12 mb-5">
                                        ...
                                        <div class="col-sm-6">
                                            ...
                                            <select id="liststatus" name="status_filter" class="form-control searchFilterm" >
                                                <option value="">All</option>
                                                @foreach (gst_status() as $status )
...
<script>
    ...
    $(document).ready(function() {
        ...
        var end = moment();
         ...
         $("#gst-status-merchant-form input[name='trans_from_date']").val(start.format('YYYY-MM-DD'));
         $("#gst-status-merchant-form input[name='trans_to_date']").val(end.format('YYYY-MM-DD'));


         $('#gst-status-merchant-form input[name="datetimes"]').daterangepicker({
    ...
    serverSide: true,
    ajax: {
        url: "{{ route('get-gststatus-merchant-list') }}",
        data: function(d) {
            d.search = $('input[type="search"]').val(),
            d.form=getJsonObject($("#gst-status-merchant-form").serializeArray())
    ...
    columns: [{
     ...
     $(".searchFilter").click(function (event) {
      ...
      $(document).on('click', '.CallMerchantGst', function(event){
          
  Swal.fire({
        title: 'Do you want to take status of Merchant Gst?',
        showDenyButton: true,
