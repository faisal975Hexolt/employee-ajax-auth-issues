        ...
        @inject('refund', 'App\Refund')
            @if(count($transactions)>0)
                ...
                <tr>
                    ...
                    <td>{{setupNumFormat((float)$transaction->transaction_adjustment_charged_per)}}</td>
                    <td>{{setupNumFormat((float)$transaction->transaction_adjustment_charged_amount)}}</td>
                    <td>{{($transaction->transaction_gst_value)}}</td>
                    <td>{{setupNumFormat((float)$transaction->transaction_gst_charged_amount)}}</td>
                    <td>{{setupNumFormat((float)$transaction->transaction_total_charged_amount)}}</td>
                    <td>{{setupNumFormat((float)$transaction->transaction_total_adjustment)}}</td>
                    ...
                    <td id="transactiongid1">
                              <?php
                                ...
                                if($refund->get_payment_refund_count($transaction->id)){
                                    $refund_detail=$refund->get_payment_refund_detail($transaction->id);
                                  
                                    $refund_status=$refund_detail[0]->refund_status;
                                    $rcount=1;
                                }
                             ?>
                        @if($rcount)
                             
                          <span class="btn btn-success btn-sm">{{ucfirst($refund_status)}}</span>
