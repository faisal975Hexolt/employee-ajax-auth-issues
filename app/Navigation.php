...
class Navigation extends Model
{
    ...
    {   
        ...
        //DB::enableQueryLog();
        $result = Navigation::with('sublinks')->where($apply_condition)->select("id","link_name","hyperlink","parent_id","hyperlinkid","icons")->orderBy("order_no",'asc')->get();
        //dd(DB::getQueryLog());
        return $result;
    }
    ...
    {
        return $this->hasMany('App\Navigation','parent_id')->where(["user_type"=>$this->user_type,"department_id"=>$this->department_type,"nav_status"=>"active"])->select("id","link_name","hyperlink","parent_id","hyperlinkid","icons")->orderBy("order_no",'asc');
    }
