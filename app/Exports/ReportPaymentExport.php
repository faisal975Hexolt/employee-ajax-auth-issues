...
class ReportPaymentExport
{
    ...
    public function export_payment($report_filter, $file_name, $logId)
    {
        ...
        $transactionResult = DB::table('live_payment')->select([
            $paymentTable . '.id',
            'transaction_gid',
            $paymentTable . '.order_id as merchantorderId',
            'transaction_type',
        ...
        // dd($report_filter, $totalRows, 'rows', $paymentTable);
        var_dump($totalRows);
        $chunkSize = 1000;
        $chunkCounter = 0;
        ...
        $transactionResult->chunk($chunkSize, function ($transactions) use ($writer, $totalRows, &$chunkCounter, $chunkSize, $logId) {
            ...
            foreach ($transactions as $transaction) {
                ...
                $row = [
                    ...
                    ucwords(\App\Classes\Helpers::payment_mode($transaction->transaction_mode)),
                    $transaction_adjustment_charged_per,
            ...
            var_dump($chunkCounter, $chunkSize, $processedRows);
            $progress = min(100, number_format(($processedRows * 100) / $totalRows, 2));

            var_dump($progress);
            // Update the progress in the log
        ...
        // Save the S3 file path and download link in the database
        DB::table('transaction_report_log')->where('id', $logId)->update(['s3_file_path' => $s3FilePath, 'progress' => 100, 'download_link' => $url]);
    ...
    public function reseller_file_export_payment($report_filter, $file_name, $logId)
    {
        var_dump('process started');
        $this->fromdate = $report_filter['from'];
        $this->todate = $report_filter['to'];
        ...
        if ($report_filter['created_merchant'] != 0) {
            $transactionResult->where($paymentTable . ".created_merchant", $report_filter['created_merchant']);
        }
    ...
    public function export_refund($report_filter, $file_name)
    {
        ...
        foreach ($results->lazy(1000) as $transaction) {
            ...
            $row = array(
                ...
                '',
                ucwords($transaction->refund_status),
    ...
    public function reseller_export_payment($report_filter, $file_name)
    {
        $this->fromdate = $report_filter['from'];
        $this->todate = $report_filter['to'];
        $payment = new \App\Payment;
        ...
        $transactionResult->orderBy($paymentTable . ".id", 'asc');
        $results = $transactionResult->distinct()->get();
        // dd($results);
        $i = 0;
        $writer = SimpleExcelWriter::streamDownload($file_name);
        $writer->addHeader($this->headings());
        foreach ($results->lazy(1000) as $transaction) {
            $transaction_adjustment_charged_per = '';
            if ($transaction->transaction_adjustment_charged_per) {
                $transaction_adjustment_charged_per = (float)$transaction->transaction_adjustment_charged_per;
            }
            $row = array(
                $transaction->created_date,
                $transaction->id,
