...
function dashborad_summary_graph(graphdata) {
    ...
    if (graphdata.payment_platform.length > 0) {



        paymentPlatLabels = [];
        paymentPlatData = [];
        $.each(graphdata.payment_platform, function (indexInArray, valueOfElement) {
            paymentPlatLabels.push(valueOfElement.merchant_device);
            paymentPlatData.push(valueOfElement.total);
        });
        ...
        $("#graph-summary-3").html('<canvas id="graph_summary_3" width="400" height="400"></canvas>');

        var ctx = document.getElementById("graph_summary_3").getContext('2d');

        var myChart = new Chart(ctx, {
            type: 'pie',

            data: {
                labels: paymentPlatLabels,
                datasets: [{
                    backgroundColor: [
                        '#A52A2A',
                        '#9932CC',
        ...
        $("#graph-summary-3").html("<div><h4>No data available.</h4><p>Tip: You could try again by selecting a different filter or date range.</p></div>");

    }

    if (graphdata.status_wise.length > 0) {
        successLabels = [];
        successData = [];
    
        $.each(graphdata.status_wise, function (indexInArray, valueOfElement) {
            successLabels.push(valueOfElement.transaction_status);
            successData.push(valueOfElement.total);
        });
