...
function ValidateRecorrect(element,error){

    if(element=='bank_name1'){
      return  validateName(element);
    }else if(element=='bank_acc_no'){
        return acceptOnlyNumeric(element,error);
    }else if(element=='comp_pan_number'){
        return ValidateCompanyPAN(element,error);
    }else if(element=='comp_cin'){
        return ValidateCompanyCIN(element,error);
    }else if(element=='comp_gst'){
        return ActivevalidateGSTno(element,error);
    }else if(element=='mer_pan_number'){
        return ValidatePAN(element,error);
    }else if(element=='mer_aadhar_number'){
        return validateAadharCard(element,error);
    }else if(element=='1otp_aadhar_number'){
        return validateOtpAadharCard(element,error);
    }
    return true;
}
...
function ValidateCompanyCIN(element,error) {
    var txtPANCard = document.getElementById(element);
    //var lblPANCard = document.getElementById(error)
    ...
    if(txtPANCard.value.length <= 21)
    {
        if (regex.test(txtPANCard.value.toUpperCase())) {
            ...
            //lblPANCard.style.visibility = "visible";
            $('#'+element).focus();
            $("#"+error).html("Invalid CIN Number").css({"color":"red"});
            return false;
        }

    }else{
        $('#'+element).focus();
        $("#"+error).html("CIN Number Must be 21 chars").css({"color":"red"});
        return false;
    }
}
...
function validateAadharCard(element,id){
    ...
    {   
        ...
        $("#"+id).html("Aadhar no should be of 12 digits").css({"color":"red"});
        return false;
    }
}

function validateOtpAadharCard(element,id){
    ...
    {   
        ...
        $("#"+id).html("OTP should not be more than 6 digits").css({"color":"red"});
        return false;
    }
}
