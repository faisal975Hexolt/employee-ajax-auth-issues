...
$("#merchant-register").submit(function (e) {
    ...
    $.ajax({
        type: "POST",
        url: base_phpurl + "/mobile-register",
        data: getJsonObject(formdata),
        dataType: "json",
        success: function (response) {
            if (response.status) {
...
$("#mobile-verification").submit(function (e) {
    ...
    $.ajax({
        type: "POST",
        url: base_phpurl + "/register",
        data: getJsonObject(formdata),
        dataType: "json",
        success: function (response) {
            if (response.status) {
                $("#ajax-success-response").html('You have register successfully.We sent you credentials mail. Please Login').css({ "color": "green" });
                setTimeout(() => {
                    window.location.href = base_phpurl + response.redirect;
                }, 2000);
...
$(document).on('keyup', '.validatebank_ifsc_code', function (element) {
    $(".loader_custom").show();
    $("#bank_ifsc_code_error").html("");
    $("#ifsc_details").html('');
    var alphainput = document.getElementById(this.id);
    ...
    if (alphainput.value.length == 11) {
        ...
        $.ajax({
            type: "GET",
            url: base_phpurl + "/merchant/bank-ifsc/reverify",
            data: { ifsc: alphainput.value },
            async: false,
            dataType: "json",
            success: function (response) {
                htmlId = 'bank_ifsc_code';
                if (response.status) {
                    $status = true;
                    $("#ifsc_details").html(response.html);

                    //$("#ajax-business-detail-info-response-message").html(response.message).css({"color":"green"});
                    $("#activate-account #" + htmlId + "_error").html(response.message).css({ "color": "green" });
                } else {
                    $("#activate-account #" + htmlId + "_error").html(response.message).css({ "color": "red" });
        ...
        return $status;
    } else if (alphainput.value.length < 11) {
        $("#bank_ifsc_code_error").html("The bank ifsc code must be at least 11 digits.").css({ "color": "red" });
        $(".loader_custom").hide();
        return false;
    } else {

        $("#bank_ifsc_code_error").html("IFSC should not be more than 11 digits").css({ "color": "red" });
        $(".loader_custom").hide();
        return false;
    }
});
...
$("#resend-mobile-sms").click(function (e) {
    $("#mobile-verification #otp_number_ajax_error").html("");
    $("#mobile-verification #otp_number").val("");
    e.preventDefault();
    $.ajax({
        type: "GET",
        ...
        success: function (response) {
            ...
            if (typeof (response.resend_exceed) != "undefined") {
                if (typeof (response.resend_exceed)) {
...
$(document).on('click', '.notifymodalclass', function (e) {

    var content = $(this).data("content");
    var title = $(this).data("title");
    var sr = $(this).data("sr");
    $("#notifymodal .modal-title").html(sr + ":" + title);
    $("#notifymodal .modal-body").html(content);
    $('#notifymodal').modal({
        show: true,
        backdrop: true
    })
...
function showMyPassswordMerchnat(elementName, iconelement) {
    var status = $("div").data("pstatus");
    x = $("#my-current-password input[name=" + elementName + "]")[0];
    if (x.type === "password") {
        x.type = "text";
...
function validateForm() {
    ...
    if (currentTab != 4) {
        ...
        var notMandatory = ["comp_gst_doc", "bank_statement", "mer_br_document"];
...
$(document).on('click', '#send_aadhar_otp', function (event) {
    var functionStatus = true;
    var x, y, i, valid = true;
    var formelements = {};
    x = document.getElementsByClassName("tab");
    //y = x[currentTab].getElementsByTagName("input");
    y = x[3].getElementsByClassName("form-control");
    for (i = 0; i < y.length; i++) {
        // If a field is empty...
        if (y[i].value == "" && !$(y[i]).prop("disabled") && !$(y[i]).hasClass("not-mandatory") && y[i].id != 'mer_name') {
            // add an "invalid" class to the field:
            y[i].className += " invalid";
            // and set the current valid status to false
            valid = false;
    ...
    if (valid) {
        if (functionStatus && validateAadharCard('mer_aadhar_number', 'mer_aadhar_number_error') && ValidatePAN('mer_pan_number', 'merpanerror') && ValidateCompanyPAN('comp_pan_number', 'comppanerror')) {

            $("#mer_aadhar_check_error").html('');
            $.ajax({
                type: "POST",
                url: base_phpurl + "/merchant/business-aadhaar/verify",
                data: getJsonObject(formelements),
                async: false,
                dataType: "json",
                success: function (response) {
                    ...
                    if (response.status) {
                        ...
                        if (response.aadhar_otp_count > 2) {
                            ...
                            $("#send_aadhar_otp").prop("disabled", true);

                            $("#mer_aadhar_verify_error").html(response.message).css({ "color": "red" });

                        } else {
                            $("#send_aadhar_otp").html('Resend OTP');
                            ...
                            setTimeout(() => {
                                $("#send_aadhar_otp").prop("disabled", false);
                            }, 10000);
...
$(document).on('click', '#verify_aadhar_otp', function (event) {
    var functionStatus = true;
    var x, y, i, valid = true;
    var formelements = {};
    x = document.getElementsByClassName("tab");
    ...
    if (valid) {
        if (functionStatus && validateOtpAadharCard('otp_aadhar_number', 'otp_aadhar_number_error') && validateAadharCard('mer_aadhar_number', 'mer_aadhar_number_error') && ValidatePAN('mer_pan_number', 'merpanerror') && ValidateCompanyPAN('comp_pan_number', 'comppanerror')) {
                ...
                success: function (response) {
                    ...
                    if (response.status) {
                        ...
                        if (response.data == 5) {
                            ...
                            $("div#otp_aadhar_view").addClass('hide');
                            $("#send_aadhar_otp").addClass("hide");
                            setTimeout(() => {
                                $("#send_aadhar_otp").prop("disabled", true);
                            }, 10010);
                            $("#verify_aadhar_otp").prop("disabled", true);
                            $("#mer_aadhar_verify_error").html('');
                            $("#mer_aadhar_check_error").html(response.message).css({ "color": "green" });
                            $("#activate-account #mer_name").val(response.user_full_name);
                            $("#activate-account #mer_name").prop("readonly", true);
                            $("#activate-account #mer_aadhar_number").prop("readonly", true);
                        } else {
                            $("#verify_aadhar_otp").prop("disabled", true);
                            $("#verify_aadhar_otp").html('Re Verify OTP');
                            $("#mer_aadhar_check_error").html(response.message).css({ "color": "red" });

                            setTimeout(() => {
                                $("#verify_aadhar_otp").prop("disabled", false);
                            }, 10000);
...
function optionItemsInvoice() {
    invoiceItemOptions = [];
    invoiceItemOptionsList = []
    $.ajax({
        url: base_phpurl + "/merchant/get-all-items",
        type: "GET",
        dataType: "json",
        ...
        success: function (response) {
            var html = '';
            var singleItem = [];
            invoiceItemOptionsList = response;;
...
$("#add-items").on("click", function (event) {
    ...
    $('#add-item-modal').modal({ show: true, keyboard: false, backdrop: 'static' });
...
function getDashboardSummary() {
    ...
    const dashFromDate = formdata.find(item => item.name === "dash_from_date");

    if (dashFromDate && dashFromDate.value === "") {
        console.log("dash_from_date is empty");
    } else {
...
function getDashboardPayout() {
    ...
    $.ajax({
        ...
        // dataType:"Json",
        success: function (response) {
            $('#dashboard_payout_summary_view').html(null);
            $('#dashboard_payout_summary_view').html(response.payout_summary_view);
...
//stote item in invoice

$("#add-item-form").submit(function (event) {
    event.preventDefault();
    var formdata = $("#add-item-form").serializeJSON();

    $("#add-item-form input[name='item_name[]']").each(function (index, element) {
        if ($(element).val() == "") {
            $(element).css({ "border": "1px solid red" });
            return false;
        }
    ...
    $("#add-item-form input[name='item_amount[]']").each(function (index, element) {
        var regnumber_rule = /^[0-9\.]+/g;
        if (!regnumber_rule.test($(element).val())) {
            $(element).css({ "border": "1px solid red" });
            return false;
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status) {
                showMessage(response.status, response.message, "add-item-response-message");
                getAllItems();
                optionItemsInvoice();
                ...
                var oldval = '';
                $(".item_name_list").each(function (index, element) {
                    oldval = ($(element).val());
                    ...
                    $.each(invoiceItemOptionsList, function (index, value) {
                        $(element).append('<option value="' + value.id + '">' + value.item_name + '</option>')
                    });
                    $(element).val(oldval);
                });
                $.wait(function () { $('#add-item-modal').modal('hide') }, 2);
...
$("#quick-link-form").submit(function (e) {
    ...
    var errormessage = {
        "paylink_amount": "Amount field should not be empty",
...
function businessDetailForm(formdata) {
    ...
    if (Object.keys(formdata).length > 0) {
        ...
        if (functionStatus && validateAadharCard('mer_aadhar_number', 'mer_aadhar_number_error') && ValidatePAN('mer_pan_number', 'merpanerror') && ValidateCompanyPAN('comp_pan_number', 'comppanerror') && ValidateCompanyCIN('comp_cin', 'comp_cin_error')) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status) {
                        ...
                        //$("#ajax-business-detail-info-response-message").html(response.message).css({"color":"green"});
                        $("#activate-account #" + mer_aadhar_number + "_error").html(response.message).css({ "color": "green" });
                    } else {
                        $("#activate-account #" + mer_aadhar_number + "_error").html(response.message).css({ "color": "red" });
...
$(document).on('submit', '#case-form-refund', function (event) {

    event.preventDefault();

    var formdata = $("#case-form-refund").serializeArray();
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status) {
                ...
                $("#case-form-refund")[0].reset();
                setTimeout(
                    function () {
                        $("#refund-transaction-model").modal('hide');
                        getMyTransactionsByDate();
                    }, 2000);
                ...
                if (typeof response.errors != "undefined" && Object.keys(response.errors).length > 0) {
                    $.each(response.errors, function (name, message) {
                        ...
                        $("#case-form-refund input[name=" + name + "]").click(function () {
                            $("#" + name + "_ajax_error").html("");
                        });
                        $("#case-form-refund select[name=" + name + "]").click(function () {
                            $("#" + name + "_ajax_error").html("");
                        });
                    });
                } else {
